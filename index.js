...
function update() {
  ...
  if (ball.y <= 0 || (ball.y + ballHeight >= boardHeight)) ball.velocityY *= -1;
  if (detectCollision(ball, player1) && (ball.x <= player1.x + player1.width) || 
    detectCollision(ball, player2) && (ball.x + ballWidth >= player2.x)) ball.velocityX *= -1;
  if (ball.x < 0) {
    player2Score++;
    resetGame(1);
  }
...
function resetGame(direction) {
  ball.velocityX += speedIncrement * Math.sign(ball.velocityX);
  ball.velocityY += speedIncrement * Math.sign(ball.velocityY);
  ball.x = boardWidth/2;
